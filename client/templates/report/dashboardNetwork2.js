...
Template.dashboardNetwork2.onCreated(function(){
  $('aside.control-sidebar').removeClass("control-sidebar-open");
  $("body").removeClass("control-sidebar-open");

  let regionTwo = m_region.find({name: "NETWORK 2"}).fetch();
  let siteTwo = site.find( { region_id : regionTwo[0]._id } );
  var devIdNetTwo = [],
      nameSiteNetTwo = [];
  if ( siteTwo.count() > 0 ) {
    _.each(siteTwo.fetch(), function(site){
      devIdNetTwo.push( site._id );
      nameSiteNetTwo.push( site.name );
    })
  } else {
    devIdNetTwo.push(0);
    nameSiteNetTwo.push(0);
  }

  // var _self = this;
  // this.subscribe("featured-sitesDevicesRegion", devIdNetTwo);
  // this.subscribe("featured-statusLink", nameSiteNetTwo);
})
...
var SIDEBAR = {
    ...
    allDevSite : function(){
      let regionNetOne = m_region.find({name: "NETWORK 2"}).fetch();
...
Template.dashboardNetwork2.helpers({
...
Template.dashboardNetwork2.events({
